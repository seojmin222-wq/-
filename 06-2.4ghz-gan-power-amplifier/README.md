...
<details>
<summary><b>01. 작업 프로세스</b></summary>
...
### 3-1. CGH40010F Bias Point 및 Stability 분석

2.4GHz 대역에서 Power Amplifier가 안정적으로 동작하도록 CGH40010F의 동작 조건을 설정하고 S-Parameter를 분석했습니다.
...
S-Parameter를 기반으로 Transistor의 Impedance를 분석하고 **Smith Chart를 이용해 LC Matching Network를 구성**했습니다.
...
Power Amplifier는 큰 입력 신호에서 비선형적으로 동작하기 때문에 기본 주파수뿐만 아니라 2차 · 3차 Harmonic이 함께 발생합니다.
...
Filter 적용 이전에는 2차 Harmonic에서도 비교적 큰 Power가 나타났기 때문에 **L과 C를 이용한 Harmonic Trap Circuit**을 추가했습니다.
...
Layout Simulation에서 2.4GHz 기본파를 유지하면서 2차 Harmonic인
...
Input Power를 증가시키며 Output Power와 Efficiency 변화를 분석했습니다.
...
Momentum Simulation에서는 Schematic 단계에서 직접 고려하기 어려운
...
이를 통해 회로도 수준의 성능이 아니라 **실제 PCB 구조를 적용했을 때에도 목표 성능을 유지할 수 있는지 검증**했습니다.
...
따라서 Schematic Simulation은 가능했지만 PCB Layout으로 변환해 EM Simulation을 수행할 수 없는 문제가 발생했습니다.
...
특히 Filter 적용 전에는 2차 Harmonic에서도 비교적 큰 Power가 나타나 출력 신호의 왜곡 가능성이 있었습니다.
